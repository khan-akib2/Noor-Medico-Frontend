...
export async function sendInquiryEmail({ name, phone, email, message }) {
  ...
  // Clean the message to extract only text if it contains large base64 image/PDF data
  let cleanMessageForUrl = message;
  if (message.includes("[PRESCRIPTION IMAGE UPLOADED]") || message.includes("[PRESCRIPTION PDF UPLOADED]")) {
    const match = message.match(/Notes:\s*([\s\S]*?)(?:\n\n<div|$)/);
    if (match && match[1]) {
      const notesText = match[1].trim();
      cleanMessageForUrl = `${message.includes("PDF") ? "[Prescription PDF]" : "[Prescription Image]"} - ${notesText}`;
    } else {
      cleanMessageForUrl = message.includes("PDF") ? "[Prescription PDF]" : "[Prescription Image]";
    }
  }
  ...
  const response = await fetch("https://api.brevo.com/v3/smtp/email", {
    ...
    body: JSON.stringify({
      ...
      subject: `New Medicine Inquiry from ${name} [Ref: #${Math.floor(1000 + Math.random() * 9000)}]`,
      htmlContent: `
        <table role="presentation" width="100%" cellspacing="0" cellpadding="0" style="background-color: #f8fafc; padding: 20px 0; width: 100%; margin: 0;">
          <tr>
            <td align="center">
              <div style="font-family: sans-serif; padding: 24px; color: #1e293b; max-width: 600px; width: 100%; border: 1px solid #e2e8f0; border-radius: 16px; background-color: #ffffff; box-shadow: 0 4px 6px -1px rgba(0, 0, 0, 0.05); text-align: left; margin: 0 auto;">
        ...
        <div style="background-color: #f8fafc; padding: 20px; border-radius: 12px; border-left: 4px solid #0f766e; margin-top: 10px;">
                 ...
                 <!-- QUICK ADMIN RESPONSES CARD -->
                 <div style="margin-top: 24px; padding: 24px 20px; border: 1px solid #e2e8f0; border-radius: 16px; background-color: #ffffff; text-align: center; box-shadow: 0 4px 6px -1px rgba(0,0,0,0.02);">
                   <p style="margin: 0 0 20px 0; font-weight: 700; font-size: 11px; text-transform: uppercase; color: #64748b; letter-spacing: 1.5px; text-align: center;">QUICK ADMIN RESPONSES</p>
                   
                   <table role="presentation" width="100%" cellspacing="0" cellpadding="0" style="margin: 0 auto; max-width: 560px;">
                     <tr>
                         ...
                         <table role="presentation" cellspacing="0" cellpadding="0" style="display: inline-block;">
                           <tr>
                             <td align="center" bgcolor="#0f766e" style="border-radius: 8px;">
                               <a href="${appUrl}/respond?status=confirmed&amp;name=${encodeURIComponent(name)}&amp;email=${encodeURIComponent(email)}&amp;phone=${encodeURIComponent(phone)}&amp;message=${encodeURIComponent(cleanMessageForUrl)}" target="_blank" style="font-family: sans-serif; font-size: 10px; font-weight: 700; color: #ffffff; text-decoration: none; padding: 12px 14px; display: inline-block; text-transform: uppercase; letter-spacing: 0.8px; border-radius: 8px;">CONFIRM ORDER</a>
                             </td>
                           </tr>
                         </table>
                       </td>
                       <td align="center" style="padding: 6px; width: 33%;">
                         <table role="presentation" cellspacing="0" cellpadding="0" style="display: inline-block;">
                           <tr>
                             <td align="center" bgcolor="#b45309" style="border-radius: 8px;">
                               <a href="${appUrl}/respond?status=delayed&amp;name=${encodeURIComponent(name)}&amp;email=${encodeURIComponent(email)}&amp;phone=${encodeURIComponent(phone)}&amp;message=${encodeURIComponent(cleanMessageForUrl)}" target="_blank" style="font-family: sans-serif; font-size: 10px; font-weight: 700; color: #ffffff; text-decoration: none; padding: 12px 14px; display: inline-block; text-transform: uppercase; letter-spacing: 0.8px; border-radius: 8px;">STOCK DELAYED / LATER</a>
                             </td>
                           </tr>
                         </table>
                       </td>
                       <td align="center" style="padding: 6px; width: 33%;">
                         <table role="presentation" cellspacing="0" cellpadding="0" style="display: inline-block;">
                           <tr>
                             <td align="center" bgcolor="#334155" style="border-radius: 8px;">
                               <a href="${appUrl}/respond?status=declined&amp;name=${encodeURIComponent(name)}&amp;email=${encodeURIComponent(email)}&amp;phone=${encodeURIComponent(phone)}&amp;message=${encodeURIComponent(cleanMessageForUrl)}" target="_blank" style="font-family: sans-serif; font-size: 10px; font-weight: 700; color: #ffffff; text-decoration: none; padding: 12px 14px; display: inline-block; text-transform: uppercase; letter-spacing: 0.8px; border-radius: 8px;">DECLINE ORDER</a>
                             </td>
                           </tr>
                         </table>
                       </td>
                   ...
                   <div style="margin-top: 20px; border-top: 1px solid #f1f5f9; padding-top: 16px; text-align: center;">
                     <p style="margin: 0; font-size: 12px; color: #64748b;">
                       Or chat directly on: <a href="${waLink}" style="color: #0f766e; text-decoration: none; font-weight: 700;">WhatsApp Support</a>
                     </p>
                   </div>
                 </div>
                ...
                <div style="margin-top: 32px; border-top: 1px solid #f1f5f9; padding-top: 16px; text-align: center;">
                  <p style="font-size: 11px; color: #94a3b8; margin: 0 0 4px 0;">
                    This inquiry was sent automatically from the Noor Medico Website.
                  </p>
                  <p style="font-size: 11px; color: #94a3b8; margin: 0;">
                    Please respond to the customer within 15 minutes.
                  </p>
                </div>
              </div>
            </td>
...
/**
 * Sends the official pharmacist response update to the client.
 */
export async function sendPharmacistResponseEmail({ email, name, subject, message }) {
  const displayName = name ? name.trim() : "Valued Customer";
  const emailSubject = subject || "Update on Your Noor Medico Inquiry";
  ...
  const htmlContent = `
    <div style="font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, Helvetica, Arial, sans-serif; max-width: 580px; margin: 0 auto; padding: 0; background-color: #f8fafc; color: #1e293b; border-radius: 24px; overflow: hidden; border: 1px solid #e2e8f0; box-shadow: 0 10px 15px -3px rgba(0,0,0,0.05);">
      ...
      <div style="padding: 40px 30px; background-color: #ffffff;">
        <h2 style="color: #0f172a; margin-top: 0; margin-bottom: 20px; font-size: 20px; font-weight: 700; text-align: center;">Pharmacist Inquiry Update</h2>
        <p style="font-size: 15px; line-height: 1.6; color: #475569; margin-bottom: 16px;">Hello ${displayName},</p>
        <div style="font-size: 15px; line-height: 1.6; color: #334155; margin-bottom: 30px; white-space: pre-wrap;">${message}</div>
        
        <div style="background-color: #f0fdfa; border-radius: 12px; padding: 16px; margin-bottom: 30px; border: 1px solid #ccfbf1; text-align: center;">
          <p style="margin: 0; font-size: 14px; color: #0f766e; font-weight: 600;">Have questions? Contact our Pharmacist directly on WhatsApp</p>
          <a href="https://wa.me/918828081398" style="margin-top: 8px; display: inline-block; background-color: #0f766e; color: #ffffff; padding: 10px 20px; border-radius: 8px; text-decoration: none; font-size: 13px; font-weight: 700;">Chat on WhatsApp</a>
        </div>
  ...
  return sendBrevoEmail({ to: email, subject: emailSubject, htmlContent });
}
